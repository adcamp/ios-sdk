...
и в методе `-(BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions:` добавьте строки инициализации:
```{.objectivec}

 ADCAdsManager *adManager = [ADCAdsManager sharedManager];
```
либо с указанием настроек
```
...
// настраиваем параметры запроса как указано в разделе "Настройка параметров OpenRTB запроса"
...
При необходимости следить за событиями времени жизни баннера можно реализовать нужные методы из протокола ADCBannerViewDelegate и установить этот объект делегaтом баннера:
```
// пример когда текущий объект реализует методы протокола ADCBannerViewDelegate
